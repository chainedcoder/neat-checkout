...
var spkFormData;
var payBtn;
var spkKey = document.getElementById("spektraPay").dataset.key;
var c2b;
var spkPhone_cache = {};

var spkIsMobile = function () {
...
function spkButtoClickEvent() {
    var spkPayBtns = document.querySelectorAll('.spk-pay-button');
    // console.log('btn', spkPayBtns);
    spkPayBtns.forEach(function (curBtn) {
        curBtn.addEventListener('click', function (event, curBtn) {
            console.log('btn', this);
            spkPayBtnTapped(this)
...
function offset(el) {
    ...
    var h = window.innerHeight - el.offsetTop

    return { top: rect.top + scrollTop, left: rect.left + scrollLeft, bottom: h }
}

function spkCloseOnTap() {
    document.getElementById("spkCheckoutDiv").onclick = function (e) {

        if (!e) e = window.event;
        console.log("clicked", e.target.id);

        target = (e.target) ? e.target : e.srcElement;
        if (spkIsMobile() && e.target.id === 'spkCheckoutFormDiv') {
            console.log("Bye", e.target.id);
            document.getElementById('spkCheckoutFormDiv').firstChild.nextSibling.classList.remove("spk-slider");
            setTimeout(() => {
                document.getElementById('spkCheckoutFormDiv').style.display = "none";
            }, 400);
            
            spkRemoveViewPortMeta()
        } 
        // else if (e.target.id === 'spkSent') {
        //     document.getElementById('spkSent').style.display = "none";
        //     spkRemoveViewPortMeta()
        // }
        //  else if (e.target.id === 'spkLoadingCon' ){
        //     document.getElementById('spkLoadingCon').style.display  = "none";
        // } 
        else if (e.target.id === 'spkCancelled') {
            document.getElementById('spkCancelled').firstChild.nextSibling.classList.remove("spk-slider");
            setTimeout(() => {
                document.getElementById('spkCancelled').style.display = "none";
            }, 400);
            spkRemoveViewPortMeta()
        }
        else if (e.target.id === 'spkFailed' || e.target.id === 'spkCancelled') {
            document.getElementById('spkFailed').firstChild.nextSibling.classList.remove("spk-slider");
            setTimeout(() => {
                document.getElementById('spkFailed').style.display = "none";
            }, 400);
            spkRemoveViewPortMeta()
        } else if (e.target.id === 'spkTimeout') {
            document.getElementById('spkTimeout').firstChild.nextSibling.classList.remove("spk-slider");
            setTimeout(() => {
                document.getElementById('spkTimeout').style.display = "none";
            }, 400);
            spkRemoveViewPortMeta()
        } else if (e.target.id === 'spkSuccess') {
            document.getElementById('spkSuccess').firstChild.nextSibling.classList.remove("spk-slider");
            setTimeout(() => {
                document.getElementById('spkSuccess').style.display = "none";
            }, 400);
            spkRemoveViewPortMeta()
        } else if (e.target.id === 'spkPayBtn') {
            spkPaySubmit(true, e.target)
        }
        spkButtoClickEvent()
    }
}

function spkAddViewPortMeta() {
    document.head.innerHTML += '<meta id="spektraMeta" name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0">';
}

function spkRemoveViewPortMeta() {
    document.getElementById('spektraMeta').outerHTML = ""
}

function spkPayBtnTapped(el) {
    spkAddViewPortMeta();
    console.log("spkPayBtnTapped.. ",el)
    spkCheckout.showForm(el);

    // var inputs = document.getElementsByTagName("input");
    // [...inputs].forEach(function (curInp){
    //     curInp.addEventListener('focus', function(event, curInp) {
    //         this.scrollIntoView(false); //Top
    //         var card = document.getElementById("spkCheckoutFormDiv").firstChild.nextSibling
    //         card.style.position = "relative";
    //         console.log("Attempting scroll to the input in focus", card.nextSibling);
    //     });

    //     curInp.addEventListener('focusout', function(event, curInp) {
    //         this.scrollIntoView(false); //Top
    //         var card = document.getElementById("spkCheckoutFormDiv").firstChild.nextSibling
    //         card.style.position = "relative";
    //         console.log("Attempting scroll to the input in focus", card.nextSibling);
    ...
    var payCharges = document.getElementById("spkCharges");
    payCharges.innerHTML = ""
    payBtn.dataset.reference = el.dataset.reference;
    payBtn.dataset.spkamount = el.dataset.amt;
    payBtn.dataset.spkkey = el.dataset.key;
    spkGetCharges(payBtn)

    // spkCheckout.showTimeout();
    try{
        setTimeout(() => {
            document.getElementById("spkCheckoutFormDiv").firstChild.nextSibling.classList.add("spk-slider");
        }, 200);
    } catch(e){
...
function spkGetCharges(payBtn) {
    ...
    spkAjax.get(url).success(function (data, xhr) {
            ...
            if (isIE) {
                ...
                var fee = person.currency === "KES" ? Math.ceil(person.fee) : person.fee
                var pay_amount = person.currency === "KES" ? Math.ceil(person.pay_amount) : person.pay_amount
                document.getElementById("spk_charges").innerText = "(" + person.currency + " " + person.amount + " + " + fee + " FEE)"
                document.getElementById("spkPayBtn").innerText = "Pay " + person.currency + " " + pay_amount
                ...
                var fee = person.currency === "KES" ? Math.ceil(person.fee) : person.fee
                var pay_amount = person.currency === "KES" ? Math.ceil(person.pay_amount) : person.pay_amount
                var origAmount = parseInt(person.amount).toLocaleString()
                document.getElementById("spkCharges").innerText = "(" + person.currency + " " + origAmount + " + " + fee.toLocaleString() + " FEE)"
                document.getElementById("spkPayBtn").innerText = "Pay " + person.currency + " " + pay_amount.toLocaleString()
...
var initScreenH =screen.height
var winHeight = window.innerHeight || document.documentElement.innerHeight || document.getElement8sByTagName('body')[0].clientHeight


function onInputFocus(){
    // document.getElementById('spk-form-card').style.marginTop = '30px'
    // var iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
    // document.getElementById('spk-form-card').style.bottom = 'auto'
    // var w = window,
    // e = document.documentElement,
    // g = document.getElement8sByTagName('body')[0],
    // x = w.innerWidth || e.clientWidth || g.clientWidth,
    // y = w.innerHeight || e.clientHeight || g.clientHeight;

    // let keyboardHeight = window.winHeight - y
    // // let keyboardHeight = window.initScreenH-screen.height
...
function onInputBlur(){
    ...
    document.getElementById('spk-form-card').style.top = 'auto'    
    document.getElementById('spk-form-card').style.bottom = 0    
}
...
function validateName(el, submit) { //Second argument determines whether to show certain errors- use if form was submitted
    ...
    // let divOffset = offset(el)

    // document.getElementById('spk-form-card').style.bottom = '-'+divOffset.bottom + 'px'

    // console.log(divOffset.left, divOffset.top, divOffset.bottom, );
...
function spkOperatorSupport() {
    var prefix = 0;
    let spkPhoneField = document.spkCheckoutForm.spkPhoneNumber
    let spkPhoneNumber = document.spkCheckoutForm.spkPhoneNumber.value.replace(/\s/g, '');

    if (document.spkCheckoutForm.spkPhoneNumber.value[0] === '0') {
        prefix = spkPhoneNumber.slice(0, 4)
    } else {
        prefix = spkPhoneNumber.slice(0, 6)
    }
    try {
        var c2b = window.spkPhone_cache[prefix].has_c2b;

        if (typeof c2b != 'undefined' && c2b === false && spkPhoneNumber.length >= 4) {
            spkPhoneField.setAttribute("valid", "false");
            spDisplayError("spkPhoneNumber_er", "Sucks! This Operator Is Unsupported Yet");
        }
    } catch (e) {
        // console.log("phone cache ? ", spkPhone_cache);
        if (document.getElementById('spkPhoneNumber_er').textContent.indexOf('Sucks') != -1) {
            spClearError('spkPhoneNumber_er');
        }

    }
    try {
        return window.spkPhone_cache[prefix].has_c2b
    }
    catch(e){
        return true;
    }
    ...
    spkAjax.post(url, data, "application/json; charset=utf-8")
        .success(function (data, xhr) {
            ...
            if (isIE) {
                ...
                window.spkPhone_cache[spkPhonePrefix] = xhr.response

                // if (responseData.has_c2b){
                //     console.log("has c2b ", responseData.has_c2b);
                //     // document.spkCheckoutForm.spkPhoneNumber.setAttribute('valid','false');
                //     // spDisplayError("spkPhoneNumber_er", "Sucks! We don't support this operator just yet.");
                //     // window.c2b = false;
                // } else {
                //     spClearError(document.spkCheckoutForm.spkPhoneNumber.id)
                //     window.c2b = true;
                // }
...
function spkPaySubmit(state, payBtn) {
    ...
    console.log(spValidator());
        if (spValidator(true) && spkOperatorSupport()) {
            window.cForm = Object.assign({}, document.getElementById("spkCheckoutForm"));
            // console.log("Cform", window.spkFormData)
            ...
            setTimeout(() => {
                document.getElementById("spkLoadingCon").firstChild.nextSibling.classList.add("spk-slider");
            }, 200);
...
//close card
function spkCloseCard(el_id) {
    document.getElementById(el_id).outerHTML = "";
    document.getElementsByClassName("spk-pay-button").style.display = "initial";
}
...
const merchantKey = window.spkKey;
...
function spkMakePayment(payBtn, cForm) {
    ...
    makePayment = function (data) {
        if (paySockOpen) {
            console.log(data)
            window.spkFormData = Object.assign({}, data);
            spkAjax.get('https://utilities.spektra.co/locations/location/my-ip/').success(function (ip){
                var data = window.spkFormData
                ...
                // console.log("Data sent is: ", checkoutData, ip, data )
    ...
    paySocket.onmessage = function (e) {
        ...
        console.log("Data received from response", data, e);
        // console.log("Data sent for payment response");
        if (data.type === "PAYMENT_REQUESTED") {
            ...
            document.getElementById('requestSentMessage').innerHTML = data.payload
            try{
                setTimeout(() => {
                    document.getElementById("spkSent").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
        
            }
            
        } else if (data.type === "TRANSACTION_TIMED_OUT") {
            ...
            try{
                setTimeout(() => {
                    document.getElementById("spkTimeout").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
        
            }
            
        } else if (data.type === "TRANSACTION_INSUFFICIENT_BALANCE") {
            console.log("Failed due to insufficient funds", data);
            document.getElementById("spkSent").outerHTML = "";
            spkCheckout.showInsufficientFunds();
            document.getElementById("spkInsufficientFunds").style.display = "block";
            try{
                setTimeout(() => {
                    document.getElementById("spkInsufficientFunds").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
            ...
            try{
                setTimeout(() => {
                    document.getElementById("spkSuccess").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
            ...
            try{
                setTimeout(() => {
                    document.getElementById("spkFailed").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
        
            }
            
        } else if (data.type === "TRANSACTION_CANCELLED") {
            console.log("Payment cancelled");
            document.getElementById("spkSent").outerHTML = "";
            spkCheckout.showCancelled();
            document.getElementById("spkCancelled").style.display = "block";
            try{
                setTimeout(() => {
                    document.getElementById("spkCancelled").firstChild.nextSibling.classList.add("spk-slider");
                }, 200);
            } catch(e){
            ...
            // console.log("Payment Failed with unkown reason", data);
            // try{
            // document.getElementById("spkSent").outerHTML = "";
            // } catch(e){
...
function spkChangeNumber(curDiv) {
    document.getElementById(curDiv).outerHTML = "";
    document.getElementById("spkCheckoutFormDiv").style.display = "block";
    document.spkCheckoutForm.spkFirstName.value = window.spkFormData.first_name;
    document.spkCheckoutForm.spkLastName.value = window.spkFormData.last_name;
    document.spkCheckoutForm.spkPhoneNumber.value = window.spkFormData.phone;

    document.spkCheckoutForm.spkPhoneNumber.focus();
    document.spkCheckoutForm.spkPhoneNumber.dispatchEvent(new KeyboardEvent('keyup',{keyCode: ' '}));
    document.spkCheckoutForm.spkPhoneNumber.dispatchEvent(new KeyboardEvent('keyup',{keyCode: ' '}));
    // spFormatNo()
}
...
(function (root, factory) {
    ...
    if (isMobile()) {
        ...
        pages.checkoutForm = "" +
            ...
            "\n" +
            "        <div id='spk-form-card' class=\"spk-card\">\n" +
            "            \n" +
            ...
            "                        FIRST NAME\n" +
            "                    </label>\n" +
            // "                    <input id=\"spkFirstName\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" onkeyup=\"validateName(this)\" name=\"spkFirstName\" type=\"text\" autocomplete=\"off\"\n" +
            "                    <input id=\"spkFirstName\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" onkeyup=\"validateName(this)\" name=\"spkFirstName\" type=\"text\" autocomplete=\"off\"\n" +
            ...
            "                        LAST NAME\n" +
            "                    </label>\n" +
            // "                    <input id=\"spkLastName\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" onkeyup=\"validateName(this)\" name=\"spkLastName\" type=\"text\" autocomplete=\"off\"\n" +
            "                    <input id=\"spkLastName\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" onkeyup=\"validateName(this)\" name=\"spkLastName\" type=\"text\" autocomplete=\"off\"\n" +
            ...
            "                    </label>\n" +
            "\n" +
            // "                    <input id=\"spkPhoneNumber\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" name=\"spkPhoneNumber\" type=\"text\" autocomplete=\"off\" placeholder=\"Enter Your Mobile Number \"\n" +
            "                    <input id=\"spkPhoneNumber\" onFocus=\"onInputFocus()\" onBlur=\"onInputBlur()\" name=\"spkPhoneNumber\" type=\"text\" autocomplete=\"off\" placeholder=\"Enter Your Mobile Number \"\n" +
        ...
        pages.requestSent = "" +
            ...
            "    \n" +
            "                    <span id=\"requestSentMessage\" class=\"\">\n" +
            "    \n" +
            "                    </span>\n" +
            "    \n" +
            "                    <br>\n" +
        ...
        pages.loading = "" +
            "<div id=\"spkLoadingCon\" class=\"spk-checkout \">\n" +
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "            \n" +
            "                    <div class=\"spk-title spk-brand-name\">\n" +
            ...
            "                            <svg xmlns=\"http://www.w3.org/2000/svg\" width=\"70\" height=\"70\" viewBox=\"0 0 30 30\" class=\"c-spinner js-spinner\">\n" +
            "                                    <title></title>\n" +
        ...
        pages.success = "" +
            ...
            "                        <p>Your Payment Is <span style=\"color:#000000; font-family: WhitneyHTF-Bold; font-size: 19;\"> <br /> SUCCESSFUL!\n" +
            "\n" +
            "                        </p>\n" +
            "                    </span>\n" +
            "\n" +
        ...
        pages.timedOut = "" +
            ...
            "                <span class=\"spk-card-content-text\"><p>Your Payment Request Has <br /> <span style=\"color:#000000; font-family: WhitneyHTF-Bold; font-size: 19;\">TIMED OUT!</span></p>\n" +
            "\n" +
            "\n" +
            "                </span>\n" +
            "\n" +
            "                <br>\n" +
            "                <button class=\"spk-card-button\" onclick=\"spkChangeNumber(\'spkTimeout\')\">Try Again</button>\n" +
            "            </div>\n" +
            "\n" +
            "        </div>\n" +
            "\n" +
        ...
        pages.failed = "" +
            ...
            "\n" +
            "                <br>\n" +
            "                <button class=\"spk-card-button\" onclick=\"spkChangeNumber(\'spkFailed\')\">Try Again</button>\n" +
            "            </div>\n" +
            "\n" +
            "        </div>\n" +
            "\n" +
        ...
        pages.cancelled = "" +
            "<div id=\"spkCancelled\" class=\"spk-checkout\">\n" +
            "\n" +
            "        <div class=\"spk-card\">\n" +
            "            \n" +
            "                <div class=\"spk-title spk-brand-name\">\n" +
            ...
            "            <div class=\"spk-card-content\">\n" +
            "                <div class=\"spk-card-icon-failed spk-failed-text\"></div>\n" +
            "                <span class=\"spk-card-content-text\"><p>Your Payment Has Been <br /> <span style=\"color:#000000; font-family: WhitneyHTF-Bold; font-size: 19;\">CANCELED!</span></p>\n" +
            "\n" +
            "\n" +
            "                </span>\n" +
            "\n" +
            "                <br>\n" +
            "                <button class=\"spk-card-button\" onclick=\"spkChangeNumber(\'spkCancelled\')\">Try Again</button>\n" +
            "            </div>\n" +
            "\n" +
            "        </div>\n" +
            "\n" +
        ...
        pages.insufficientFunds = "" +
            "<div id=\"spkInsufficientFunds\" class=\"spk-checkout\">\n" +
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "                <div class=\"spk-title spk-brand-name\">\n" +
            "        \n" +
            ...
            "                <div class=\"spk-card-content\">\n" +
            "                    <div class=\"spk-card-icon-failed spk-failed-text\"></div>\n" +
            "                    <span class=\"spk-card-content-text\"><p>You Have Insufficient Funds <br /> <span style=\"color:#000000; font-family: WhitneyHTF-Bold; font-size: 19;\">PLEASE TOP UP!</span></p>\n" +
            "                    </span>\n" +
            "                    <button class=\"spk-card-button\" onclick=\"spkChangeNumber(\'spkInsufficientFunds\')\">Change Number</button>\n" +
            "    \n" +
            "                    <br>\n" +
            "                </div>\n" +
            "    \n" +
        ...
        pages.checkoutForm = "" +
            ...
            "    <div id=\"spkCheckoutFormDiv\" class=\"ps_checkout ps_form\">\n" +
            "\n" +
            "        <div class=\"spk-card\">\n" +
            "            <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkCheckoutFormDiv')\">\n" +
        ...
        pages.requestSent = "" +
            ...
            "\n" +
            "        <div class=\"spk-card\">\n" +
            "            <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkSent')\">\n" +
            "                <svg width=\"18px\" height=\"18px\" viewBox=\"0 0 14 14\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\"\n" +
            "                    xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n" +
            "                    <!-- Generator: Sketch 51.2 (57519) - http://www.bohemiancoding.com/sketch -->\n" +
            "                    <title>close</title>\n" +
        ...
        pages.loading = "" +
            ...
            "    <div id=\"spkLoadingCon\" class=\"ps_checkout\">\n" +
            "\n" +
            "        <div class=\"spk-card \">\n" +
            "            <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkLoadingCon')\">\n" +
            "                <svg width=\"18px\" height=\"18px\" viewBox=\"0 0 14 14\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\"\n" +
            "                    xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n" +
            "                    <!-- Generator: Sketch 51.2 (57519) - http://www.bohemiancoding.com/sketch -->\n" +
            "                    <title>close</title>\n" +
        ...
        pages.success = "" +
            ...
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "                <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkSuccess')\">\n" +
            "                    <svg width=\"18px\" height=\"18px\" viewBox=\"0 0 14 14\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\"\n" +
            "                        xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n" +
            "                        <!-- Generator: Sketch 51.2 (57519) - http://www.bohemiancoding.com/sketch -->\n" +
            "                        <title>close</title>\n" +
        ...
        pages.timedOut = "" +
            ...
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "                <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkTimeout')\">\n" +
            ...
            "    \n" +
            "        </div>\n" +
            "        <!-- End of request timeout section -->";

        pages.failed = "" +
            "<!-- Request failed section -->\n" +
            "        <div id=\"spkFailed\" class=\"ps_checkout\">\n" +
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "                <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkFailed')\">\n" +
            "                            <svg width=\"18px\" height=\"18px\" viewBox=\"0 0 14 14\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n" +
            "                            <!-- Generator: Sketch 51.2 (57519) - http://www.bohemiancoding.com/sketch -->\n" +
            "                            <title>close</title>\n" +
            "                            <desc>Created with Sketch.</desc>\n" +
            ...
            "                <div class=\"spk-card-content\">\n" +
            "                    <div class=\"spk-card-icon-failed spk-failed-text\"></div>\n" +
            "                    <span class=\"spk-card-content-text\"><p>Your Payment Request Has <br /> <span style=\"color:#000000; font-family: WhitneyHTF-Bold; font-size: 19;\">FAILED!</span></p>\n" +
            "    \n" +
            "    \n" +
            "                    </span>\n" +
            "    \n" +
        ...
        pages.cancelled = "" +
            "<!-- Request failed section -->\n" +
            "        <div id=\"spkCancelled\" class=\"ps_checkout\">\n" +
            "\n" +
            "            <div class=\"spk-card\">\n" +
            "                <span class=\"spk-card-close\" onclick=\"spkCloseCard('spkCancelled')\">\n" +
            "                            <svg width=\"18px\" height=\"18px\" viewBox=\"0 0 14 14\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n" +
            "                            <!-- Generator: Sketch 51.2 (57519) - http://www.bohemiancoding.com/sketch -->\n" +
            "                            <title>close</title>\n" +
            "                            <desc>Created with Sketch.</desc>\n" +
        ...
        pages.insufficientFunds = "" +
            "<div id=\"spkInsufficientFunds\" class=\"ps_checkout\">\n" +
            "\n" +
            "            <div class=\"spk-card\">\n" +
    ...
    var selector = function (codeString, stylescss) {
        ...
        {
        document.getElementById('spkCheckoutDiv').style.display = '';
        }
        else if(window.orientation==90)
        {
        document.getElementById('spkCheckoutDiv').style.display = 'none';
        }
        // root.spkGetCharges();
    }
    var payForm = function (codeString, btn, stylescss) {
        ...
        try {
            ...
            document.body.innerHTML += '<div id="spkCheckoutDiv"  class="spk-form">' + "<style>" + stylescss + "</style>" + codeString + '</div>';
        }
    ...
    exports.showCancelled = function () {
        return selector(pages.cancelled, pages.stylescss);
    };
    exports.showInsufficientFunds = function () {
        return selector(pages.insufficientFunds, pages.stylescss);
    };
